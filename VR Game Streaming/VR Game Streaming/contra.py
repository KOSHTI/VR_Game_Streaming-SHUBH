...
while True:
	...
	if res.pose_landmarks and inFrame(finalres):
		if not(isInit):
			...
			if isJump(finalres[0].y*480): #press f down only if f is not already down
			...
			if isRunning(): ##  running down the s key
			...
			if isShoot(finalres): # press d key for shooting
